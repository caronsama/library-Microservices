<template>
  <div class="home" style="padding: 10px">
    ...
    <el-table :data="tableData" stripe border="true" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="bookstatus" label="书本信用积分" />
      <el-table-column prop="alow" label="是否允许借阅" >
        <template v-slot="{ row }">
          <span v-if="row.alow === '1'">允许借阅</span>
          <span v-else-if="row.alow === '0'">不允许借阅</span>
        </template>
      </el-table-column>
      <el-table-column fixed="right" label="操作">
        <!-- <template v-slot="scope">
          ...
          <el-button  size="mini" @click ="noAlow(scope.row.id)" style="margin-left: 10px" type="success">禁止借阅</el-button>
          <el-popconfirm title="确认删除?" @confirm="handleDelete(scope.row.id)">
            <template #reference>
              <el-button type="danger" size="mini" style="margin-top: 10px; margin-left: 50px">删除用户</el-button>
            </template>
          </el-popconfirm>
        </template> -->
        <template v-slot="scope">
          <div style="display: flex; flex-direction: column;">
            <el-button size="mini" @click="handleEdit(scope.row)">审核/编辑个人信息</el-button>
            <el-button size="mini" @click="handleAlow(scope.row.id)" style="margin-top: 10px;"
              type="success">允许借阅</el-button>
            <el-button size="mini" @click="noAlow(scope.row.id)" style="margin-top: 10px;"
              type="warning">禁止借阅</el-button>
            <el-popconfirm title="确认删除?" @confirm="handleDelete(scope.row.id)">
              <template #reference>
...
export default {
  ...
  methods: {
    ...
    deleteBatch() {
      ...
      request.post("/user/deleteBatchUser", this.ids).then(res => {
        if (res.code === '0') {
    ...
    handleAlow(id) {
      request.put("user/accreditBorrow/" + id).then(res => {
        console.log(res)
        if (res.code == 0) {
    ...
    noAlow(id) {
      request.put("user/NoBorrow/" + id).then(res => {
        console.log(res)
        if (res.code == 0) {
          ElMessage.success("撤销授权")
        }
        else
          ElMessage.error(res.msg)
        this.load()
